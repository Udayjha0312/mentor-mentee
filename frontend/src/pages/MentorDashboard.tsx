...
function MentorDashboardPage() {
  ...
  const highlights = useMemo(
    () => [
      { label: 'Assigned mentees', value: stats?.assignedMentees || 0, tone: 'from-sky-500 to-indigo-500' },
      { label: 'Pending reviews', value: stats?.pendingReviews || 0, tone: 'from-fuchsia-500 to-purple-500' },
      { label: 'Completed tasks', value: stats?.completedTasks || 0, tone: 'from-emerald-500 to-cyan-500' }
    ],
    [stats]
  );

  const spotlightRows = [
    { name: 'Mentee A', risk: 'High', status: 'Needs follow-up', last: '2h ago' },
    { name: 'Mentee B', risk: 'Medium', status: 'Stable', last: '5h ago' },
    { name: 'Mentee C', risk: 'Medium', status: 'In review', last: '1d ago' },
    { name: 'Mentee D', risk: 'High', status: 'Meeting scheduled', last: '3d ago' },
    { name: 'Mentee E', risk: 'Medium', status: 'On track', last: '4d ago' }
  ];
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-[36px] border border-slate-200 bg-gradient-to-br from-white via-slate-100 to-slate-50 p-8 shadow-lg shadow-slate-200/30">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            ...
            <h1 className="mt-3 text-4xl font-semibold text-slate-950">Hello, {user.name}. Ready to guide your mentees today?</h1>
            <p className="mt-4 text-slate-600">See the latest engagement metrics, at-risk insights, and quick actions in one polished workspace view.</p>
          </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.5fr_1fr]">
        <div className="space-y-6">
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-sm uppercase tracking-[0.3em] text-slate-500">Mentee Spotlight</p>
                <h2 className="mt-2 text-2xl font-semibold text-slate-950">Critical alerts and next steps</h2>
              </div>
                ...
                <tbody>
                  {spotlightRows.map((row) => (
                    <tr key={row.name} className="border-t border-slate-200 hover:bg-slate-100">
                      ...
                      <td className="px-4 py-4 text-sm text-slate-700">{row.risk}</td>
                      <td className="px-4 py-4 text-sm text-slate-700">{row.status}</td>
                      <td className="px-4 py-4 text-sm text-slate-500">{row.last}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="space-y-6">
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <p className="text-sm uppercase tracking-[0.3em] text-slate-500">Quick insights</p>
            <div className="mt-8 grid gap-4">
              <div className="rounded-3xl bg-slate-950/5 p-5">
                <p className="text-sm text-slate-500">Low engagement</p>
                <div className="mt-3 h-3 overflow-hidden rounded-full bg-slate-200">
                  <div className="h-full w-36 rounded-full bg-slate-900" />
                </div>
              </div>
              <div className="rounded-3xl bg-slate-950/5 p-5">
                <p className="text-sm text-slate-500">Task delays</p>
                <div className="mt-3 h-3 overflow-hidden rounded-full bg-slate-200">
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-sm uppercase tracking-[0.3em] text-slate-500">Mentoring health</p>
                <h2 className="mt-2 text-2xl font-semibold text-slate-950">Team progress</h2>
              </div>
              <span className="rounded-full bg-slate-900 px-3 py-2 text-sm font-semibold text-white">Stable</span>
            </div>
